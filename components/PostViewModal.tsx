...
const PostViewModal: React.FC<PostViewModalProps> = ({ post, isOpen, onClose, currentUser, onDelete }) => {
    ...
    console.log('PostViewModal render - isOpen:', isOpen, 'post:', post);

    if (!isOpen || !post) {
        console.log('PostViewModal not showing - isOpen:', isOpen, 'post:', post);
        return null;
    }

    console.log('PostViewModal SHOWING!');
    ...
    const handleDelete = async () => {
        console.log('[DEBUG] Attempting delete. Post ID:', post.id);
        console.log('[DEBUG] Current User:', currentUser);
        console.log('[DEBUG] Is Author?', currentUser?.id === post.authorId);
        ...
        try {
            ...
            console.log('[DEBUG] Delete response:', { error, count });
            ...
            if (onDelete) {
                console.log('[DEBUG] Calling onDelete prop');
                onDelete(post.id);
            } else {
            ...
            console.error('[DEBUG] Catch Error deleting post:', error);
            alert('Erro ao excluir post: ' + (error.message || JSON.stringify(error)));
        }
    };
